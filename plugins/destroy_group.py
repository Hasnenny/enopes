...
async def destroy_all_group(c: Client, m: Message):
    ...
    await m.reply_text("هل انت متاكد\n",
...
async def destroyyes(c: Client, m: CallbackQuery):
    if sudo(m):
        ...
        await c.answer_callback_query(m.id, text="ميخصك حب", show_alert=True)
...
async def destroyno(c: Client, m: CallbackQuery):
    if sudo(m):
        await m.message.delete()
        await m.message.reply_text("همزين فكرت بذا الشيء")
    else:
        await c.answer_callback_query(m.id, text="ميخصك بشيء", show_alert=True)
