...
async def sourcegame(c: Client, m: CallbackQuery):
    ...
    if m.from_user.id != int(a[1]):
        await c.answer_callback_query(m.id, text="صاحب الامر هو فقط من يستطيع الضغط على الزر", show_alert=True)
        return
    game_message = """↯︙ اوامر ( فتح + قفل ) الالعاب 
– – – – – –
↯︙ ( الالعاب )

↯︙ او ~ صراحه ~ مريم ~ عقاب
↯︙ نسبه جمالي ~ نسبه الحب ~ كشف الكذب ~ نمله
↯︙ صرصار ~ خنزير ~ كره السله ~ النشال 
↯︙ النرد ~ كت تويت ~ كت تويت بالصور ~ الاسرع 
↯︙ التشابه ~ المختلف ~ الرياضيات ~ الانجليزي 
↯︙ الامثله ~ العكس ~ الحزوره ~ المعاني
...
async def games(c: Client, m: Message):
    ...
    if m.text == "زوجني" or m.text == "زواج" or m.text == "الزواج" and m.reply_to_message:
        global mid
        mid = m.id
        keyboard = InlineKeyboardMarkup(inline_keyboard=[
