...
↞ عشان تفعلني ارفعني اشراف وارسل تفعيل.
                ...
                keyboard = InlineKeyboardMarkup(inline_keyboard=[
                    [InlineKeyboardButton("ضيفني لـ مجموعتك 🧚",
    ...
    if not confirm_group_test(m):
        try:
            ...
            if check[1]:
                await confirm_group(c, m)
                return
            await m.reply_text("عليك رفع البوت مشرف اولا \n")
            return
        except Exception as e:
...
########################################################################################################################
    ...
    if m.text == "تعطيل":
        if sudo2(m):
            ...
            await m.reply_text("⇜ هاذا الامر يخص المطور وما فوق\n")
            return
    ...
    if m.text == "تحديث" or m.text == "ريفريش" or m.text == "رفع الادمنيه":
        if manager(m):
            ...
            u = await m.reply_text("جاري التحديث\n")
            await u.delete()
        ...
        if lock_lockbye_test(m):
            try:
                if get_db_addbyegroup(m.chat.id) is None:
                    t = f"""
ليش غادرت {m.left_chat_member.mention}
❬ سد الباب وراك >
                         """
                    await m.reply_text(t)
                    m.text = "del"
...
########################################################################################################################
    ...
    if m.text == "اذاعه بالتوجيه للمجموعات" or m.text == "اذاعه بالتوجيه للكروباتات":
        if secsudo(m):
...
########################################################################################################################
    ...
    if m.text == "بوت" or m.text == "البوت":
        if sudo2(m):
            await m.reply_text("وشتبي!!!؟\n")
        else:
            if manager(m):
                await m.reply_text("تفضل اخي !!؟\n")
            else:
                if constractors(m):
                    await m.reply_text("ترا اسمي بلاك\n")
                else:
                    if admin(m):
                        await m.reply_text("بلاكك اسمي افهم\n")
                    else:
                        if special(m):
                            await m.reply_text("هلا تفضل\n")
                        else:
                            if get_db_botname() is None:
                                botname = "زيرو"
                            else:
                                botname = get_db_botname()
                            await m.reply_text("اسمي " + botname + " ضلعي")

    if m.text == (get_db_botname() or "زيرو"):
        texting = ["تفضل " + (get_db_botname() or "زيرو") + " تفضل كلبي",
                   "ها حبي" + (get_db_botname() or "زيرو") + "❤️",
                   "انطم",
                   "وقح",
                   "شتبي",
                   "لا تناديني",
                   ...
                   "عيوني",
                   "خلاص يا بابا",
                   "وش بغيت" + (get_db_botname() or "زيرو") + "😘",
                   "هلا حياتي"
                   ]
        await m.reply_text(random.choice(texting))
        return

    if m.text == "بوت غادر" or m.text == (get_db_botname() or "زيرو") + " غادر":
        if secsudo(m):
            try:
                await m.reply_text("تم حبيبي\n")
                del_db_checkgroup(m.chat.id)
                del_db_managerall(m.chat.id)
                del_db_constractorsall(m.chat.id)
                del_db_adminall(m.chat.id)
...
########################################################################################################################
    ...
    if m.text == "السورس" or m.text == "سورس" or m.text == "يا سورس":
        ...
        keyboard = InlineKeyboardMarkup(inline_keyboard=[
            ...
            [ InlineKeyboardButton ( "ل مـشاعركـم ؟", url=f"https://t.me/ee_re1" )], 
            [ InlineKeyboardButton ( "مبرمج السورس", url=f"https://t.me/hhthth" )], 
        [InlineKeyboardButton("ضيفني لـ مجموعتك 🧚",
                              url=f"https://t.me/{get_bot_information()[1]}?startgroup=new")],
        ])
        await m.reply_text("""
- [ل مـشاعركـم ؟](t.me/ee_re1)
- [𝗔𝗦𝗞 𝗧𝗢 𝗠𝗘](t.me/hhthth)
- [Hasnen | حسنين](t.me/hhthth)

For the first time, Source Zero was launched 
    ...
    if m.text == "مطور" or m.text == "المطور" or m.text == "مطور البوت":
        ...
        try:
            ...
            await m.reply_text("مطور السورس \n✧ مالك البوت\n", reply_markup=keyboard),
            await c.send_message(sudoers[0], f"نداء لك ايها المبرمج\n بواسطة {sender_name}\n يوم {today}\n الساعه {clock}\n اسم الكروب {name_chat}\n ايدي الكروب {id_chat}\n عدد اعضاء الكروب {num_member}\n رابط المسج {message_link}\n الرابط {link_group}", parse_mode=enums.ParseMode.MARKDOWN)
        else:
    ...
    if m.text == "حسنين":
        ...
        try:
            ...
            await c.send_message("7031471363", f"نداء لك ايها المبرمج\n بواسطة {sender_name}\n يوم {today}\n الساعه {clock}\n اسم الكروب {name_chat}\n ايدي الكروب {id_chat}\n عدد اعضاء الكروب {num_member}\n رابط المسج {message_link}\n الرابط {link_group}", parse_mode=enums.ParseMode.MARKDOWN)
        else:
            async for photo in c.get_chat_photos("7031471363", limit=1):
                    ...
                    await c.send_message("7031471363", f"نداء لك ايها المبرمج\n بواسطة {sender_name}\n يوم {today}\n الساعه {clock}\n اسم الكروب {name_chat}\n ايدي الكروب {id_chat}\n عدد اعضاء الكروب {num_member}\n رابط المسج {message_link}\n الرابط {link_group}"
arse_mode=enums.ParseMode.MARKDOWN)
        return
...
- نوع الكشف : ( كشف بالمعرف ) .
    ...
    if m.text == "الترحيب":
        if admin(m):
            if get_db_addwelcomegroup(m.chat.id) is None:
                t = f"""
- اطلق من يدخل نورتنا يحبيبي ❤️‍🔥 :  - {m.from_user.mention} ..
                        """
                await m.reply_text(t)
            else:
                for per in get_db_addwelcomegroup(m.chat.id):
